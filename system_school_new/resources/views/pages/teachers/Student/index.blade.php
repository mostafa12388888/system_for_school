...
@foreach( $student->attendance()->where('attendence_date',date('Y-m-d'))->get() as $attendence)
                                       {{ $attendence->attendence_status == 1 ? 'checked' : '' }}
                                    @endforeach
                            ...
                            <label class="ml-4 block text-gray-500 font-semibold">
                                ...
                                @foreach( $student->attendance()->where('attendence_date',date('Y-m-d'))->get() as $attendence)
                                       {{ $attendence->attendence_status == 0 ? 'checked' : '' }}
                                    @endforeach
                                       value="absent">
